...
impl CameraObjectFilter {
    ...
    pub fn new(
        ...
        let decrement_slope:f32 = Self::MAXIMUM * (1. - is_healthy_after / full_health_ticks) / full_health_to_unhealty_time;
    ...
    pub fn object_seen(&mut self, time: Instant) {
        ...
        let new_health:f32 = self.health + self.INCREMENT - (time - self.time_last_update).as_secs_f32() * self.DECREMENT_SLOPE;
    ...
    pub fn object_invisible(&mut self, time: Instant) {
        ...
        self.health = f32::max(0., self.health - (time - self.time_last_update).as_secs_f32() * self.DECREMENT_SLOPE);
    }
}